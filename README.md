...
- Phase 26 local audit journal crash-like truncation, tamper, concurrent append, sync, invalid-filesystem, and simulated disk-full validation implemented.
...
The audit durability boundary adds local append locking, file flush plus `sync_all`, append/reopen replay validation, crash-like truncated JSONL replay rejection, tamper rejection, concurrent append replay validation, invalid filesystem fail-closed checks, disk-full error classification, and a simulated disk-full append probe that confirms journal state is not advanced after failed persistence. It does not simulate every production filesystem failure, physical disk-full behavior, retention/rotation policy, service-manager restart behavior, stale lock recovery, live trading, signing, broadcasts, withdrawals, bridges, exchange/RPC calls, wallet custody, or production readiness.
