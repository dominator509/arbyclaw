...
15. Export new Phase 27 opportunity, planner-handoff, and candidate-trace types.
16. Surface the refined opportunity-engine scope in CLI status.
17. Update governance docs, structure validation, and production gap tracking.
18. Run the standard validation sequence.
...
- CLI validation can run the local historical fixture corpus and local opportunity planner handoff and fail closed on failed replay windows, missing candidate audit/state traces, planner handoff failures, adapter-submission flags, or forbidden side-effect flags.
