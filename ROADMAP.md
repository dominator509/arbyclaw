...
- Active phase: Phase 27 - Opportunity Depth, Inventory, and Transfer-Risk Modeling implemented for local deterministic opportunity realism; next required work is broader scenario validation, triangular path search, deployment-host/runtime validation, and external sandbox/live evidence beyond current Rust/CI gates
...
| 27 | Opportunity Depth, Inventory, and Transfer-Risk Modeling | Implemented as local deterministic opportunity realism; external calibration and full triangular path search deferred | +0% direct | Adds optional caller-supplied order-book depth walking, paper inventory caps, transfer-risk profile penalties, and candidate liquidity/transfer-risk records without live data calls, real transfers, signing, broadcasts, or execution. |
