...
- Active phase: Phase 129 - Communications Provider Adapter Readiness Gate implemented to require a typed local communications provider adapter readiness review in the operator-surface aggregate while preserving no outbound delivery request, outbound network use, provider calls, token loading, message delivery, live execution, signing, broadcasts, or production-readiness claims; next required work remains real REST/WebSocket provider implementation, live DEX/RPC adapters, provider-backed market-data latency/rate-limit/outage/bad-data validation, real provider/API fee validation, external account-tier confirmation, gas/RPC/network fee validation, withdrawal-cost validation, broader external/deployment replay corpora, production load/security/backtest validation, daemon-hosted observability/exporter/alert validation, real persistent dashboard deployment/browser integration/security validation, real outbound communications adapters, external sandbox/live calibration, custody-backed signer implementation, operator-controlled deployment-host/service-manager execution, deployment-host durability/filesystem validation, physical disk-full execution, rollback/incident execution, production image publishing, service deployment, and external evidence beyond current Rust/CI/local gates
- Active sub-roadmap: `PHASE_129_SUBROADMAP.md`
...
- Current implementation status: Minimal Rust workspace, typed config, reference-only secret boundary, policy/audit/state/runtime lifecycle gates, deterministic local market-data/fee/paper/CEX/DEX/opportunity/planner/adapter boundaries, local communications/dashboard/observability/testing/packaging/handoff gates, and aggregate local validation scripts exist. The operator-surface aggregate now composes 16 local components, including the communications delivery-provider boundary CLI, communications provider submission preflight CLI, observability provider-boundary CLI, observability provider submission preflight CLI, and dashboard persistent local-host readiness CLI for non-secret accounting of missing real provider delivery/exporter/log-shipping/alert-delivery/platform-identity/provider-validation and real dashboard daemon/browser/security/deployment evidence without real delivery, export, log shipping, alert delivery, persistent public hosting, browser secret retention, or live controls, plus the dashboard session lifecycle CLI for non-secret hosted-session/CSRF reference, revocation, read-only-role, rate-limit, and loopback-only accounting. The connector aggregate gate composes 25 local connector/provider components, including typed local market-data live-provider and fee live-provider boundary reviews, Web3 provider nonce reconciliation, Web3 sandbox/live discrepancy calibration, plus CEX and DEX/Web3 live-adapter boundary reviews that validate local REST/WebSocket/RPC request-plan, lifecycle transcript, balance transcript, credential-scope, rate-limit, exchange-matching, unsigned payload, nonce, non-broadcast, fee reconciliation, and provider-backed fee evidence accounting prerequisites while blocking on sandbox/live/provider-backed evidence. The current deployment-host runtime wrapper and Phase 28/90/109/110/112/114/116/117/118 aggregate deployment-runtime gate compose 44 local runtime/deployment probes, including 31 nested runtime/helper probes and thirteen sanitized runtime/deployment transcript validators, with static deployment hardening/config smoke validation, runtime config reload, SQLite WAL schema migration, deployment config/log redaction, dashboard session lifecycle, dashboard loopback runtime, communications outbox, communications delivery-provider boundary, communications provider-submission preflight, observability metrics runtime, observability provider boundary, observability provider-submission preflight, runtime load-profile review, and production-runtime preflight enforcement while failing on unsafe side-effect flags. The deployment evidence bundle now includes 34 local components and the checklist requires the deployment-host dashboard runtime, dashboard session lifecycle, dashboard loopback runtime, communications outbox, and observability provider-submission components. The packaging aggregate gate directly requires the production-intent container validator, including Docker validation completion, hardened read-only/no-network smoke, dropped capabilities, no-new-privileges, and explicit service-installation non-claims. The hardening-core aggregate now composes thirteen local components by requiring packaging/deployment, execution-path aggregate, operator-surface aggregate, opportunity-scenario aggregate, connector-scenario aggregate, deployment evidence checklist, dependency-license policy, secret-boundary audit, secret backup/restore, withdrawal policy boundary, signer boundary audit, destination boundary audit, and policy-decision audit validation, with deployment-checklist assertions for the 34-component deployment evidence bundle, zero missing required components, preserved external-evidence blockers, and no service actions, file mutation, secret loading, external calls, alerts, live execution, embedded artifact contents, or readiness claim. The handoff-candidate aggregate gate composes execution-path, operator-surface, opportunity-scenario, connector-scenario, hardening-core, deployment-evidence checklist, and local handoff audit validation in one local candidate surface. Local and GitHub Actions evidence covers current structure, formatting, workspace compilation, tests, clippy, locked release build, aggregate packaging/deployment gate validation, dependency audit, dependency license policy validation, SBOM generation, local-SARIF SAST, example image scan, static example systemd-unit checks, secret-pattern scan, deployment evidence checklist artifact generation, and hardening evidence indexing. Live trading, production container publishing/service deployment, production systemd/ARM deployment validation, external validation runner execution beyond local deterministic gates, real fuzzing engines, broader external/deployment opportunity scenario-corpus execution, real external backtest corpus execution, real observability runtime, real dashboard hosting, outbound messaging integrations, external adapter submission, real exchange-specific live connectors, DEX RPC adapters, wallet signer, transaction broadcasts, custody backend, deployment-host durability validation, real deployment-host permission execution, physical disk-full and retention/rotation execution evidence, operator-controlled service-manager lifecycle execution evidence, deployment-host schema migration execution under service lifecycle, actual rollback execution evidence, incident-response execution evidence, daemon failure-capture execution evidence, external sandbox/live calibration evidence, and production execution logic are not implemented.
...
| 18 | Agentic Handoff Package | Implemented as deterministic model/docs plus local audit/state boundary with a full local-surface handoff-candidate aggregate gate; external agent execution not performed | +0% direct | Codex/Cursor/Jules/Claude/human handoff package records, prompts, checklists, `validate-agentic-handoff-audit` local replay gate, and `validate_agentic_handoff_candidate_gate.py` over handoff audit, execution-path, operator-surface, opportunity-scenario, connector-scenario, hardening-core, and deployment-evidence checklist gates added; no external agents executed. |
...
| 30 | Connector Scenario Aggregate Gate | Implemented as local deterministic aggregate connector scenario validation; live exchange/RPC and external sandbox evidence deferred | +0% direct | Adds `scripts/validate_connector_scenario_gate.py`, which now composes 25 local market-data, fee, CEX request-plan, CEX balance-snapshot, DEX request-plan, DEX response-transcript, DEX transaction-lifecycle transcript, DEX protocol-risk review, Web3 provider nonce reconciliation, Web3 sandbox/live discrepancy calibration, and CEX/DEX lifecycle CLIs, including market-data quality assessment, bad-data rejection review, market-data live-provider boundary review, fee schedule reconciliation review, fee live-provider boundary review, paid-provider evaluation, historical persistence, and provider rate-limit/outage reconciliation review, and fails closed if live network use, credential loading, account queries, WebSocket opening, provider calls, external submission, RPC calls, signing/broadcast, live execution, or production readiness are reported. |
...
- Created `PHASE_101_SUBROADMAP.md`.
- Added a CI preparation step before the strict handoff-candidate aggregate gate to pull `aquasec/trivy:latest`.
- Preserved the production-container validator's Dockerized Trivy `--pull never` scan-container behavior.
...
Met for CI fresh-runner preparation of the strict local handoff aggregate only. GitHub Actions run `28741475080` passed for commit `0330ad02c6263bd3b11e51b2d9b1bcac6b3c63d2`, including the Trivy image preparation step and strict handoff-candidate aggregate gate. Production readiness remains unclaimed.

## Phase 102 - Structure Manifest Consistency Gate

### Implementation

- Created `PHASE_102_SUBROADMAP.md`.
- Updated `scripts/generate_structure_manifest.py` so generated manifest context names the current generated-inventory responsibility instead of a stale Phase 55 paragraph.
...
- Refreshed `STRUCTURE_MANIFEST.md` so Phase 86 through Phase 102 subroadmaps and current required file hashes are present.
...
Met for local/CI structure-manifest consistency enforcement only. This does not close live integration, production deployment, service-manager, physical disk-full, rollback/incident execution, or production-readiness blockers.

## Phase 103 - Market Data Live Provider Boundary Gate

### Implementation

- Created `PHASE_103_SUBROADMAP.md`.
...
- Added `review_market_data_live_provider_boundary()` with explicit missing-evidence blockers for provider sessions, provider-backed latency, provider-backed rate-limit/outage, and provider-backed bad-data validation.
- Added focused Rust tests and `arb-agent validate-market-data-live-provider-boundary`.
- Required the new component in `scripts/validate_connector_scenario_gate.py`, raising the aggregate to 22 local connector/provider components.
...
Met for local live-provider boundary accounting only. Real REST/WebSocket providers, provider-backed latency/rate-limit/outage/bad-data validation, credentials, exchange/RPC calls, deployment-host validation, and production readiness remain unclaimed.

## Phase 104 - Fee Live Provider Boundary Gate

### Implementation

- Created `PHASE_104_SUBROADMAP.md`.
- Added `FeeLiveProviderBoundaryReviewRequest`, `FeeLiveProviderBoundaryReviewReport`, and `FeeLiveProviderBoundaryReviewStatus`.
- Added `review_fee_live_provider_boundary()` with explicit missing-evidence blockers for provider-backed maker/taker fee, account-tier, gas/RPC/network fee, and withdrawal-cost validation.
- Added focused Rust tests and `arb-agent validate-fee-live-provider-boundary`.
- Required the new component in `scripts/validate_connector_scenario_gate.py`, raising the aggregate to 23 local connector/provider components.
...
Met for local provider-backed fee boundary accounting only. Real provider/API fee validation, account-tier confirmation, gas/RPC/network fee validation, withdrawal-cost validation, credentials, exchange/RPC calls, deployment-host validation, and production readiness remain unclaimed.

## Phase 105 - Web3 Connector Aggregate Coverage Gate

### Implementation

- Created `PHASE_105_SUBROADMAP.md`.
- Required existing `arb-agent validate-web3-provider-nonce-reconciliation` in `scripts/validate_connector_scenario_gate.py`.
- Required existing `arb-agent validate-web3-sandbox-live-discrepancy-calibration` in `scripts/validate_connector_scenario_gate.py`.
...
- Added generic dangerous-key detection for `rpc-called` and `external-call-performed`.
- Raised the connector scenario aggregate to 25 local connector/provider components.
...
Met for local connector aggregate coverage only. Live RPC adapters, real provider-backed nonce retrieval, production nonce/confirmation management, external sandbox/live calibration evidence, custody-backed signing, broadcasts, bridges, deployment-host validation, and production readiness remain unclaimed.

## Phase 108 - Observability Provider Boundary Gate

### Implementation

- Created `PHASE_108_SUBROADMAP.md`.
- Added `ObservabilityProviderBoundaryReviewRequest`, `ObservabilityProviderBoundaryReviewReport`, and `ObservabilityProviderBoundaryReviewStatus`.
- Added `review_observability_provider_boundary()` with explicit missing-evidence blockers for exporter sessions, log shipping, alert delivery, deployment-host runtime operation, and production metrics authentication.
- Added append-only audit journal and SQLite WAL checkpoint helpers for the provider boundary report.
- Added focused Rust tests and `arb-agent validate-observability-provider-boundary --workspace <fresh-dir>`.
- Required the new component in `scripts/validate_operator_surface_gate.py`, raising the aggregate to 13 local operator-surface components.

### Validation

```text
cargo test -p arb-core observability_provider_boundary -- --nocapture
cargo test -p arb-agent observability_provider_boundary -- --nocapture
cargo run -p arb-agent -- validate-observability-provider-boundary --workspace <fresh-dir>
```

### Exit Criteria

Met for local observability provider-boundary accounting only. Real exporter sessions, external log shipping, real alert delivery, daemon-hosted observability runtime, deployment-host metrics authentication, public exposure validation, service-manager execution, external AppSec review, and production readiness remain unclaimed.

## Phase 109 - Deployment Observability Provider Boundary Gate

### Implementation

- Created `PHASE_109_SUBROADMAP.md`.
...
- Required that component in `scripts/validate_deployment_evidence_checklist.py`.
- Added provider-boundary wrapper assertions to `scripts/validate_deployment_runtime_gate.py`.

### Validation

```text
python scripts/validate_deployment_host_runtime.py --run-observability-provider-boundary --observability-provider-boundary-workspace <fresh-dir> --json
...
Met for deployment-facing local observability provider-boundary accounting only. Real deployment-host exporter sessions, log shipping, alert delivery, daemon-hosted runtime operation, production metrics authentication, service-manager execution, external AppSec review, and production readiness remain unclaimed.

## Phase 110 - Deployment Communications Delivery Provider Gate

### Implementation

- Created `PHASE_110_SUBROADMAP.md`.
...
- Required that component in `scripts/validate_deployment_evidence_checklist.py`.
- Added delivery-provider wrapper assertions to `scripts/validate_deployment_runtime_gate.py`.

### Validation

```text
python scripts/validate_deployment_host_runtime.py --run-communications-delivery-provider-boundary --communications-delivery-provider-workspace <fresh-dir> --json
python scripts/validate_deployment_runtime_gate.py --json
python scripts/validate_deployment_evidence_bundle.py --json
python scripts/validate_deployment_evidence_checklist.py --json
```
...
Met for deployment-facing local communications delivery-provider accounting only. Real messaging adapters, platform-token handling, real provider delivery, provider-side rate-limit reconciliation, outage/backoff validation, production platform identity authorization, service-manager execution, external AppSec review, and production readiness remain unclaimed.

## Phase 111 - Communications Provider Submission Preflight Gate

### Implementation

- Created `PHASE_111_SUBROADMAP.md`.
...
- Added `review_communication_provider_submission_preflight()` over the existing delivery-provider boundary report.
- Added focused fail-closed Rust tests.
- Added `arb-agent validate-communications-provider-submission-preflight --workspace <fresh-dir>`.
- Required the new component in `scripts/validate_operator_surface_gate.py`, raising the aggregate to 14 local operator-surface components.

### Validation

```text
cargo test -p arb-core communication_provider_submission_preflight -- --nocapture
cargo run -p arb-agent -- validate-communications-provider-submission-preflight --workspace <fresh-dir>
python scripts/validate_operator_surface_gate.py
```

### Exit Criteria

Met for local provider submission preflight accounting only. Real provider calls, message delivery, platform-token loading, real outbound communications adapters, provider-side validation, production platform identity authorization, external AppSec review, and production readiness remain unclaimed.

## Phase 112 - Deployment Communications Provider Submission Preflight Gate

### Implementation

- Created `PHASE_112_SUBROADMAP.md`.
...
```text
python scripts/validate_deployment_host_runtime.py --run-communications-provider-submission-preflight --communications-provider-submission-workspace <fresh-dir> --json
python scripts/validate_deployment_runtime_gate.py --json
python scripts/validate_deployment_evidence_bundle.py --json
python scripts/validate_deployment_evidence_checklist.py --json
```
...
Met for deployment-facing local communications provider-submission preflight accounting only. Real provider calls, message delivery, platform-token loading, real outbound communications adapters, provider-side validation, production platform identity authorization, service-manager execution, external AppSec review, and production readiness remain unclaimed.

## Phase 115 - Deployment Evidence Bundle Dashboard Runtime Component Gate

### Implementation

- Created `PHASE_115_SUBROADMAP.md`.
...
Met for direct deployment evidence bundle/checklist enforcement of the existing local dashboard runtime wrapper only. Real persistent dashboard hosting, browser credential/session handling, CSRF token serving from a daemon, public-exposure validation, service-manager execution, external security review, and production readiness remain unclaimed.

## Phase 116 - Deployment Dashboard Loopback Runtime Gate

### Implementation

- Created `PHASE_116_SUBROADMAP.md`.
- Added `--run-dashboard-loopback-runtime` and `--dashboard-loopback-workspace` to `scripts/validate_deployment_host_runtime.py`.
- Added dashboard loopback runtime assertions to `scripts/validate_deployment_runtime_gate.py`, raising the aggregate to 42 local runtime/deployment components and 29 nested runtime/helper components.
...
```text
python scripts/validate_deployment_host_runtime.py --run-dashboard-loopback-runtime --dashboard-loopback-workspace <fresh-dir> --json
python scripts/validate_deployment_runtime_gate.py --json
python scripts/validate_deployment_evidence_bundle.py --json
python scripts/validate_deployment_evidence_checklist.py --json
```
...
Met for deployment-facing local dashboard loopback runtime enforcement only. Real persistent dashboard hosting, browser credential/session handling, CSRF token serving from a daemon, public-exposure validation, service-manager execution, external security review, and production readiness remain unclaimed.

## Phase 117 - Deployment Dashboard Session Lifecycle Gate

### Implementation

- Created `PHASE_117_SUBROADMAP.md`.
- Added `--run-dashboard-session-lifecycle` and `--dashboard-session-workspace` to `scripts/validate_deployment_host_runtime.py`.
- Added dashboard session lifecycle assertions to `scripts/validate_deployment_runtime_gate.py`, raising the aggregate to 43 local runtime/deployment components and 30 nested runtime/helper components.
...
```text
python scripts/validate_deployment_host_runtime.py --run-dashboard-session-lifecycle --dashboard-session-workspace <fresh-dir> --json
python scripts/validate_deployment_runtime_gate.py --json
python scripts/validate_deployment_evidence_bundle.py --json
python scripts/validate_deployment_evidence_checklist.py --json
```
...
Met for deployment-facing local dashboard session lifecycle enforcement only. Real persistent dashboard hosting, browser credential/session handling, CSRF token serving from a daemon, public-exposure validation, service-manager execution, external security review, and production readiness remain unclaimed.

## Phase 118 - Deployment Communications Outbox Gate

### Implementation

- Created `PHASE_118_SUBROADMAP.md`.
- Added `--run-communications-outbox` and `--communications-outbox-workspace` to `scripts/validate_deployment_host_runtime.py`.
- Added communications outbox assertions to `scripts/validate_deployment_runtime_gate.py`, raising the aggregate to 44 local runtime/deployment components and 31 nested runtime/helper components.
...
```text
python scripts/validate_deployment_host_runtime.py --run-communications-outbox --communications-outbox-workspace <fresh-dir> --json
python scripts/validate_deployment_runtime_gate.py --json
python scripts/validate_deployment_evidence_bundle.py --json
python scripts/validate_deployment_evidence_checklist.py --json
```
...
Met for deployment-facing local communications outbox enforcement only. Real provider delivery, platform-token loading, outbound network delivery, provider-side rate-limit/outage validation, service-manager execution, external security review, and production readiness remain unclaimed.

## Phase 119 - Hardening Core Secret Backup Restore Gate

### Implementation

- Created `PHASE_119_SUBROADMAP.md`.
- Added `validate-secret-backup-restore --workspace <fresh-dir>` as a required `secret_backup_restore` component in `scripts/validate_hardening_core_gate.py`.
- Added aggregate assertions for ready and blocked backup/restore reviews, blocked validation codes, sanitized references, backup payload shape, restore verification, review-window validity, fail-closed audit/state behavior, replayed audit records, SQLite checkpoint recovery, and no unsafe secret/live side-effect flags.
- Advanced `scripts/validate_structure.py` to require phase files through Phase 119.
...
Met for local non-secret hardening-core enforcement of secret backup/restore review evidence only. Real secret backup or restore execution, credential loading, plaintext decryption, keystore writes, OS keyring calls, external credential restoration, signing, broadcasts, live execution, deployment mutation, external custody review, and production readiness remain unclaimed.

## Phase 120 - Hardening Core Withdrawal Policy Gate

### Implementation

- Created `PHASE_120_SUBROADMAP.md`.
- Added `validate-withdrawal-policy-boundary --workspace <fresh-dir>` as a required `withdrawal_policy_boundary` component in `scripts/validate_hardening_core_gate.py`.
- Added aggregate assertions for config guard, strategy flag guard, strategy intent guard, trust-contract guard, destination allowlist guard, signing-boundary guard, fail-closed audit/state behavior, replayed audit record, SQLite checkpoint recovery, and no external submission, secret recording, or production-readiness flags.
- Advanced `scripts/validate_structure.py` to require phase files through Phase 120.
...
Met for local hardening-core enforcement of fail-closed withdrawal policy boundary evidence only. Real withdrawal execution, signer/custody implementation, credential loading, destination ownership validation, external exchange/wallet/RPC/provider calls, signing, broadcasts, live execution, deployment mutation, and production readiness remain unclaimed.

## Phase 121 - Hardening Core Signer Boundary Gate

### Implementation

- Created `PHASE_121_SUBROADMAP.md`.
- Added `validate-signer-boundary-audit --workspace <fresh-dir>` as a required `signer_boundary_audit` component in `scripts/validate_hardening_core_gate.py`.
...
- Advanced `scripts/validate_structure.py` to require phase files through Phase 121.
...
Met for local hardening-core enforcement of signer boundary audit evidence only. Real signer/custody implementation, key loading, plaintext decryption, hardware wallet access, signing, broadcasts, RPC calls, live execution, deployment mutation, and production readiness remain unclaimed.

## Phase 122 - Hardening Core Destination Boundary Gate

### Implementation

- Created `PHASE_122_SUBROADMAP.md`.
- Added `validate-destination-boundary-audit --workspace <fresh-dir>` as a required `destination_boundary_audit` component in `scripts/validate_hardening_core_gate.py`.
- Added aggregate assertions for allowlist version presence, enabled destination entry count, referenced evidence count, destination allowlist and ownership-review audit fail-closed behavior, state fail-closed behavior, replayed audit records, SQLite checkpoint recovery, and no chain ownership verification, signer material loading, challenge signing, or production-readiness flags.
- Advanced `scripts/validate_structure.py` to require phase files through Phase 122.
...
Met for local hardening-core enforcement of destination boundary audit evidence only. Real address ownership proof execution, signer material loading, challenge signing, transfers, withdrawals, RPC calls, live execution, deployment mutation, and production readiness remain unclaimed.

## Phase 123 - Hardening Core Execution Path Aggregate Gate

### Implementation

- Created `PHASE_123_SUBROADMAP.md`.
...
- Added aggregate assertions for 18 execution-path components, full pass status, no unsafe side-effect flags, no external calls, no external submission, no signer material loading, no plaintext decryption, no signing, no broadcast, no live execution, and no production-readiness flag.
- Advanced `scripts/validate_structure.py` to require phase files through Phase 123.

### Validation

```text
python scripts/validate_execution_path_gate.py --json
python scripts/validate_hardening_core_gate.py --json
python scripts/validate_structure.py
```
...
Met for local hardening-core enforcement of the existing execution-path aggregate only. Real exchange/RPC adapter execution, custody-backed signing, external submission, provider-backed nonce/simulation validation, sandbox/live discrepancy evidence, deployment mutation, and production readiness remain unclaimed.

## Phase 124 - Hardening Core Operator Surface Aggregate Gate

### Implementation

- Created `PHASE_124_SUBROADMAP.md`.
...
- Added aggregate assertions for 15 operator-surface components, full pass status, no unsafe side-effect flags, no outbound network use, no public network exposure, no service-manager action, no external submission, no signing or broadcast, no live execution, and no production-readiness flag.
- Advanced `scripts/validate_structure.py` to require phase files through Phase 124.
...
Met for local hardening-core enforcement of the existing operator-surface aggregate only. Real outbound communications delivery, persistent dashboard hosting, public exposure validation, daemon-hosted observability/exporter/alert validation, service-manager execution, external AppSec review, and production readiness remain unclaimed.

## Phase 125 - Hardening Core Opportunity Scenario Aggregate Gate

### Implementation

- Created `PHASE_125_SUBROADMAP.md`.
...
- Added aggregate assertions for 14 opportunity-scenario components, full pass status, replay-latency review enforcement, validation-coverage review enforcement, candidate coverage, no unsafe side-effect flags, no external calls, no external data downloads, no adapter submission, no external fuzzer invocation, no live network use, no signing or broadcast, no live execution, and no production-readiness flag.
- Advanced `scripts/validate_structure.py` to require phase files through Phase 125.

### Validation

```text
python scripts/validate_opportunity_scenario_gate.py --json
python scripts/validate_hardening_core_gate.py --json
python scripts/validate_structure.py
```
...
Met for local hardening-core enforcement of the existing opportunity-scenario aggregate only. Broader external/deployment scenario-corpus execution, external sandbox/live calibration evidence, live/provider-backed market-data validation, external fuzzing-engine and broader production backtest execution, production runtime validation, and production readiness remain unclaimed.

## Phase 126 - Hardening Core Connector Scenario Aggregate Gate

### Implementation

- Created `PHASE_126_SUBROADMAP.md`.
...
Met for local hardening-core enforcement of the existing connector-scenario aggregate only. Live REST/WebSocket exchange adapters, provider-backed market-data/fee validation, sandbox/live order lifecycle calibration, live DEX/RPC simulation/router validation, deployment-host connector validation, and production readiness remain unclaimed.

## Phase 127 - Hardening Core Deployment Evidence Checklist Gate

### Implementation

- Created `PHASE_127_SUBROADMAP.md`.
...
- Advanced `scripts/validate_structure.py` to require phase files through Phase 127.
...
Met for local hardening-core enforcement of the existing deployment evidence checklist only. Operator-controlled service lifecycle execution, deployment-host backup/restore, graceful-shutdown, audit/SQLite, schema-migration, disk-full, retention/rotation, rollback, incident-response, daemon failure-capture, human production-readiness review, and production readiness remain unclaimed.

## Phase 128 - Dashboard Persistent Local Host Readiness Gate

### Implementation

- Created `PHASE_128_SUBROADMAP.md`.
- Added `DashboardPersistentLocalHostReadinessRequest`, `DashboardPersistentLocalHostReadinessReport`, and `DashboardPersistentLocalHostReadinessStatus`.
- Added `review_dashboard_persistent_local_host_readiness()` plus append-only audit and SQLite WAL checkpoint helpers.
- Added `arb-agent validate-dashboard-persistent-local-host --workspace <fresh-dir>`.
- Added focused Rust tests for ready, blocked, and audit/state replay paths.
- Required `dashboard_persistent_local_host_cli` in `scripts/validate_operator_surface_gate.py`, raising the operator-surface aggregate to 16 local components.
- Updated `scripts/validate_hardening_core_gate.py` to require the 16-component operator-surface aggregate.
- Advanced `scripts/validate_structure.py` to require phase files through Phase 128.

## Phase 129 - Communications Provider Adapter Readiness Gate

Status: Implemented locally; production communications delivery remains blocked pending real provider validation and operator-owned external evidence.

Implemented:
- Created `PHASE_129_SUBROADMAP.md`.
- Added typed communications provider adapter readiness request/report/status models and `review_communication_provider_adapter_readiness` in `arb-core`.
- Added local-only tests covering blocked-pending provider adapter validation and fail-closed side-effect input.
- Added `arb-agent validate-communications-provider-adapter-readiness --workspace <fresh-dir>` with append-only audit replay and SQLite WAL checkpoint recovery prerequisites.
- Required the new CLI in `scripts/validate_operator_surface_gate.py` and raised the hardening-core operator-surface component count to 17.
- Advanced `scripts/validate_structure.py` to require phase files through Phase 129.

Remaining:
- Real outbound communications adapters, real provider-side validation, authenticated delivery receipts, provider response transcripts, platform identity authorization, production deployment/browser/security validation, external sandbox/live calibration, and production readiness remain open blockers.
...
Met for the local persistent dashboard host readiness boundary only. Real daemon-hosted dashboard supervision, real browser authentication/session integration, external dashboard security review, deployment-host public exposure denial evidence, production dashboard deployment/runtime validation, and production readiness remain unclaimed.
