...
| 28 | Deployment Runtime Aggregate Gate | Implemented as local deterministic aggregate runtime/deployment validation; deployment-host execution evidence deferred | +0% direct | Adds `scripts/validate_deployment_runtime_gate.py`, which composes 28 local runtime/deployment probes by combining the deployment-host runtime helper with ten sanitized runtime/deployment transcript/rehearsal validators, explicitly enforces the embedded production-runtime preflight contract from local runtime smoke, requires the service-manager lifecycle transcript to include sanitized concurrent lifecycle evidence fields, requires the service-manager lifecycle rehearsal to validate ordered lifecycle semantics, requires the deployment permission transcript to include sanitized runtime-write permission-denial fields, requires the deployment SQLite schema migration transcript to include sanitized migration evidence fields, and fails closed if any nested report or transcript summary claims service-manager action, external calls, live execution, secret loading, public exposure, telemetry export, outbound alert/network delivery, production-path mutation, deployment-path mutation, or production readiness. |
...
- The aggregate gate runs the existing `scripts/validate_deployment_host_runtime.py` helper with 18 local-only runtime/deployment components enabled against fresh `target/` workspaces, layers nine sanitized runtime/deployment transcript validators on top for a 27-probe aggregate, and now explicitly enforces the local production-runtime preflight status, blocker count, and no-readiness evidence fields emitted by runtime smoke.
