...
fn run_with_args(args: impl IntoIterator<Item = String>) -> Result<(), AgentCliError> {
    ...
    match args.next().as_deref() {
        Some("--config") => {
            ...
            println!("opportunity-engine: {OPPORTUNITY_ENGINE_VERSION} available for deterministic discovery/ranking plus local replay checks with supplied depth, paper inventory, transfer-risk, and triangular records only; live execution pending");
