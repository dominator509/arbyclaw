...
- Completion criteria: Local command/notification records, local mocked platform command-ingress validations, local remote command envelope validations, local channel-adapter validations, local channel-session validations, local platform-adapter reviews, the local communications runtime CLI gate, and the deployment-host wrapper remain locally authorized or control-reviewed where applicable, fail closed for disabled, unauthenticated, unauthorized, replayed, stale, unsafe, provider-unavailable, missing future-delivery preconditions, or side-effectful remote sources, redacted, durably journaled, replayable, wrapper-reportable, and SQLite-checkpointed; every future real remote command and notification is authenticated where remote, authorized by platform identity, rate-limited, outage-tested, redacted, idempotent, kill-switch controlled, audit/state preflighted, and fail-closed without enabling direct live execution.
