...
- This validates local structure, formatting, compilation, tests, linting, local runtime-smoke CLI behavior, example-container Docker/Trivy smoke behavior, static example systemd-unit checks, and disposable-container `systemd-analyze` syntax checks only. It does not validate production deployment, live funds, live exchange/RPC integrations, signing, broadcasts, production containers, deployment-host systemd behavior, ARM, CI, penetration testing, load testing, rollback drills, incident drills, deployment-host audit behavior, physical disk-full behavior, retention/rotation execution behavior, service-manager restart execution behavior, external hardening, or production readiness.
