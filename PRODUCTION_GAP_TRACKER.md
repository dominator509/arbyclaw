...
- This closes the local coding gap for caller-supplied depth, paper inventory, and transfer-risk modeling only. It does not close full triangular path search, broader fixture replay, external sandbox/live calibration evidence, live exchange/RPC validation, custody/signing, or production-readiness blockers.
...
- Description: Phase 9 added deterministic opportunity-engine models, freshness checks, fee-aware cross-venue top-of-book discovery, and deterministic ranking. Phase 27 adds local caller-supplied order-book depth walking, paper inventory caps, transfer-risk profiles, and candidate liquidity/transfer-risk records. Full triangular path discovery, broader scenario replay, planner integration validation, and external sandbox/live validation remain incomplete.
- Why incomplete: Phase 9 intentionally added the smallest safe discovery/ranking boundary without execution-intent generation, live data, live connectors, or advanced route search. Phase 27 reduces the local realism gap but still depends on caller-supplied local records and does not provide live data, full triangular route search, external calibration, or production validation.
...
- Exact future validation required: keep unit tests, fee-aware ROI tests, stale-data denial tests, depth/slippage tests, inventory constraints, and settlement-latency risk tests passing; add replay tests, false-positive tests, full triangular-route tests, planner handoff tests, and backtesting over broader historical fixtures.
