...
- Branch: `main`
- Latest validated commit: `d0132b75ba842c832ef501410daf9ce8d5f04cba`
- Workflow run: `https://github.com/dominator509/arbyclaw/actions/runs/26271152507`
- Result: passed.
...
- Pending next CI validation: a lightweight `hardening-evidence-index` artifact has been added to summarize non-secret hardening evidence artifact names, producing job results, run URL, commit, and explicit non-claims for easier operator lookup.
...
- GitHub Actions `cargo build --release --locked`, `cargo audit`, CycloneDX SBOM generation, CodeQL Rust SAST local-SARIF analysis, example container image build, Trivy image-scan gate, and Gitleaks secret-pattern scan gate passed in run `https://github.com/dominator509/arbyclaw/actions/runs/26271152507` for commit `d0132b75ba842c832ef501410daf9ce8d5f04cba`.
...
- A lightweight CI `hardening-evidence-index` artifact has been added for the next workflow run so non-secret hardening evidence artifact names and producing job results are easier to locate without changing runtime behavior.
