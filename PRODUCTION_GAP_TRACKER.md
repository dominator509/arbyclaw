...
- GitHub Actions `cargo build --release --locked`, `cargo audit`, CycloneDX SBOM generation, CodeQL Rust SAST local-SARIF analysis, example container image build, Trivy image-scan gate, Gitleaks secret-pattern scan gate, and lightweight hardening evidence index artifact upload passed in run `https://github.com/dominator509/arbyclaw/actions/runs/26291038924` for commit `5228bee94f7d0189c5850189e3c9ca983d3b3f1e`.
...
- GitHub Actions run `https://github.com/dominator509/arbyclaw/actions/runs/26291038924` uploaded the lightweight `hardening-evidence-index` artifact, plus `codeql-sarif-evidence`, `trivy-image-scan-evidence`, `gitleaks-secret-scan-evidence`, and the Docker Buildx build record artifact, making non-secret hardening evidence easier to locate without changing runtime behavior.
- Pending next CI validation: the `hardening-evidence-index` job now also writes a GitHub Step Summary with the same non-secret artifact pointers, producing job results, run URL, commit, and explicit non-claims for easier lookup from the workflow page.
