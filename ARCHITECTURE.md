...
- Phase 78 update: Local market-data bad-data rejection review now composes acceptable baseline quality, stale-data rejection, excessive-spread rejection, insufficient-depth rejection, capture-latency rejection, a bad-data fixture floor, and unresolved external provider evidence. This remains local-only and does not implement live REST/WebSocket providers, provider credentials, provider-backed bad-data validation, external latency/data-quality measurement, adapter submission, signing, broadcasts, live execution, or production readiness.
