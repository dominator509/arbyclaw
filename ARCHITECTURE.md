...
- Opportunity engine has current workspace Rust validation evidence and now models local caller-supplied order-book depth, paper inventory caps, transfer-risk penalties, same-venue triangular path discovery, local replay/false-positive reports, a built-in local regression corpus, local historical fixture replay aggregation, and local replay CLI validation commands wired into CI, but broader external/deployment scenario-corpus validation and external sandbox/live calibration remain missing
