...
- Current status override: Phase 129 communications provider adapter readiness gate is implemented. The operator-surface aggregate gate now requires `arb-agent validate-communications-provider-adapter-readiness --workspace <fresh-dir>` as a seventeenth local component, composing provider submission preflight, sanitized request-plan shape, endpoint/auth references, payload template, idempotency, rate-limit budget, retry/backoff, outage circuit breaker, response transcript requirement, and delivery receipt requirement while preserving no outbound delivery request, outbound network use, provider calls, token loading, message delivery, live execution, signing, broadcasts, or production-readiness claim. This remains local/CI evidence only and does not approve credential loading, plaintext decryption, keystore writes, OS keyring calls, external secret restoration, production dashboard hosting, browser credential/session handling, live trading, public exposure, real provider calls, real exporter sessions, log shipping, outbound alert delivery, platform-token loading, outbound network use, real outbound communications delivery, signing, broadcasts, secrets, service-manager execution, withdrawals, transfers, or production readiness.

- Status: Phase 105 Web3 connector aggregate coverage gate is implemented after Phase 104 fee live provider boundary gate, Phase 103 market-data live provider boundary gate, Phase 102 structure manifest consistency gate, Phase 101 CI handoff aggregate container scan preparation, Phase 100 handoff candidate full local surface gate enforcement, and earlier local connector/runtime validation phases. The current architecture includes typed local live market-data provider and fee live-provider boundary reviews, typed local DEX/Web3 and CEX live-adapter boundary reviews, local Web3 provider nonce reconciliation, local Web3 sandbox/live discrepancy calibration; typed local production-runtime preflight accounting; direct deployment evidence bundle/checklist enforcement for lifecycle plan, deployment-host runtime plan, retention preflight, service-manager rehearsal, rollback/incident drill plans, deployment disk-full, retention, permission-denial, audit/SQLite, backup/restore, graceful-shutdown, SQLite schema migration, rollback, incident-response, failure-capture, response-drill, config redaction, log redaction, static hardening/config smoke, and observability metrics runtime wrapper evidence; deployment-host wrapper and aggregate deployment-runtime enforcement for static deployment hardening/config smoke validation through the real `arb-agent --config` path; deployment-host wrapper and aggregate deployment-runtime enforcement for typed local observability metrics runtime validation; typed local observability metrics runtime, dashboard loopback runtime, communications outbox, deployment log/config redaction, SQLite WAL schema migration, runtime config reload, fee schedule reconciliation/live-provider boundary accounting, market-data bad-data/provider reconciliation/live-provider boundary accounting, runtime load profile, deployment transcript/rehearsal, packaging, hardening, and handoff validation boundaries. The packaging/deployment aggregate gate directly requires the production-intent container validator, including Docker validation completion, hardened read-only/no-network smoke, dropped capabilities, no-new-privileges, and service-installation non-claims propagated through hardening and handoff aggregates. The handoff-candidate aggregate gate composes execution-path, operator-surface, opportunity-scenario, connector-scenario, hardening-core, deployment-evidence checklist, and local handoff audit validation, and CI now pulls the Dockerized Trivy scanner image before running that strict aggregate on fresh runners. The structure validator now enforces required-file byte/hash consistency against the generated structure manifest so required governance, source, script, workflow, and phase files cannot silently drift from `STRUCTURE_MANIFEST.md`. Current workspace Rust/CI validation evidence exists, while real production load testing, daemon-hosted observability runtime/exporter/alert operation, real persistent dashboard hosting, real outbound communications delivery, live exchange/RPC adapter implementation, live REST/WebSocket market-data providers, provider-backed market-data session/latency/rate-limit/outage/bad-data validation, real provider-backed nonce retrieval, provider-backed validation, real provider/API fee validation, external account-tier confirmation, gas/RPC/network fee validation, withdrawal-cost validation, broader external/deployment scenario-corpus validation, production image publishing, service installation, actual deployment-host config reload/start/stop/restart validation, actual deployment-host backup/restore execution under service lifecycle and load, actual deployment-host graceful-shutdown execution, deployment-host audit validation under service lifecycle, deployment-host schema migration execution under service lifecycle, deployment-host config loading under service lifecycle, deployment-host log/audit redaction under service lifecycle, real deployment-host runtime-write permission-denial execution, physical disk-full evidence, deployment-host retention/rotation execution evidence, operator-controlled service-manager lifecycle execution evidence, actual rollback execution evidence, actual incident-response execution evidence, daemon failure-capture execution evidence, external sandbox/live calibration evidence, deployment-host durability validation, external agent execution, and real production hardening validation remain deferred.
...
- Phase 104 update: The fee framework now exposes `FeeLiveProviderBoundaryReviewRequest` / `FeeLiveProviderBoundaryReviewReport` plus `arb-agent validate-fee-live-provider-boundary`, and the connector scenario aggregate gate now includes 23 local components with fee live-provider boundary enforcement while preserving no live provider calls, RPC calls, credential loading, signing, broadcasts, withdrawals, external submission, live execution, or production-readiness claim.
- Phase 105 update: The connector scenario aggregate gate now includes 25 local components by requiring existing Web3 provider nonce reconciliation and sandbox/live discrepancy calibration CLIs, with aggregate assertions for nonce readiness, provider snapshot readiness, pending nonce uniqueness, observation references, sample-size readiness, and deviation-limit checks while preserving no external calls, RPC calls, credential loading, signer material, signing, broadcasts, live execution, or production-readiness claim.
- Phase 106 update: The operator-surface aggregate gate now includes 11 local components by requiring `validate-communications-delivery-provider-boundary`, which composes local communications runtime prerequisites with explicit missing real provider delivery, provider rate-limit, provider outage/backoff, and production platform identity evidence while preserving no provider calls, token loading, message delivery, remote command enablement, live execution, signing, broadcasts, or production-readiness claim.
- Phase 107 update: The operator-surface aggregate gate now includes 12 local components by requiring `validate-dashboard-session-lifecycle`, which records non-secret hosted-session and CSRF references, local auth/authorization, CSRF lifecycle, revocation support, read-only role posture, rate-limit posture, loopback-only scope, audit replay, and SQLite checkpoint recovery without persistent hosting, browser credential storage, public exposure, live controls, external submission, live execution, or production-readiness claim.
- Phase 108 update: The operator-surface aggregate gate now includes 13 local components by requiring `validate-observability-provider-boundary`, which composes local observability operations review, export dry-run, alert-route dispatch, endpoint preflight, and bounded metrics runtime prerequisites, records missing exporter-session/log-shipping/alert-delivery/deployment-host-runtime/production-metrics-auth evidence, and persists audit/SQLite recovery without exporter sessions, log shipping, alert delivery, public exposure, service-manager action, external submission, sensitive-material loading, live execution, or production-readiness claim.
- Phase 109 update: Deployment-facing runtime/evidence gates now require the same local observability provider boundary through `scripts/validate_deployment_host_runtime.py --run-observability-provider-boundary`, `scripts/validate_deployment_runtime_gate.py`, `scripts/validate_deployment_evidence_bundle.py`, and `scripts/validate_deployment_evidence_checklist.py`, preserving missing real provider/runtime evidence as blockers without exporter sessions, log shipping, alert delivery, public exposure, service-manager actions, external calls, sensitive-material loading, live execution, or readiness claims.
- Phase 110 update: Deployment-facing runtime/evidence gates now require the same local communications delivery-provider boundary through `scripts/validate_deployment_host_runtime.py --run-communications-delivery-provider-boundary`, `scripts/validate_deployment_runtime_gate.py`, `scripts/validate_deployment_evidence_bundle.py`, and `scripts/validate_deployment_evidence_checklist.py`, preserving missing real delivery/rate-limit/outage/platform-identity evidence as blockers without provider calls, message delivery, token loading, public exposure, service-manager actions, external calls, sensitive-material loading, live execution, signing, broadcasts, or readiness claims.
- Phase 111 update: The communications subsystem now exposes `CommunicationProviderSubmissionPreflightRequest` / `CommunicationProviderSubmissionPreflightReport` plus `arb-agent validate-communications-provider-submission-preflight --workspace <fresh-dir>`, and the operator-surface aggregate now includes 14 components by requiring that preflight. The preflight composes the existing delivery-provider boundary and requires local kill-switch, audit/state, idempotency, rate-limit, outage/backoff, and payload-redaction controls while blocking on missing real provider validation evidence without provider calls, message delivery, token loading, outbound network use, live execution, signing, broadcasts, or readiness claims.
- Phase 112 update: Deployment-facing runtime/evidence gates now require the same local communications provider-submission preflight through `scripts/validate_deployment_host_runtime.py --run-communications-provider-submission-preflight`, `scripts/validate_deployment_runtime_gate.py`, `scripts/validate_deployment_evidence_bundle.py`, and `scripts/validate_deployment_evidence_checklist.py`, raising the aggregate deployment-runtime gate to 40 local runtime/deployment components while preserving no provider calls, message delivery, token loading, outbound network use, service-manager actions, external calls, live execution, signing, broadcasts, or readiness claims.
- Phase 113 update: The observability subsystem now exposes `ObservabilityProviderSubmissionPreflightRequest` / `ObservabilityProviderSubmissionPreflightReport` plus `arb-agent validate-observability-provider-submission-preflight --workspace <fresh-dir>`, and the operator-surface aggregate now includes 15 components by requiring that preflight. The preflight composes the existing observability provider-boundary report and requires local telemetry/export kill switch, audit/state, idempotency, exporter backpressure, alert-delivery authorization, and telemetry redaction controls while blocking on missing real provider validation evidence without exporter sessions, log shipping, alert delivery, public exposure, service-manager action, sensitive-material loading, live execution, signing, broadcasts, or readiness claims.
- Phase 114 update: Deployment-facing runtime/evidence gates now require the same local observability provider-submission preflight through `scripts/validate_deployment_host_runtime.py --run-observability-provider-submission-preflight`, `scripts/validate_deployment_runtime_gate.py`, `scripts/validate_deployment_evidence_bundle.py`, and `scripts/validate_deployment_evidence_checklist.py`, raising the aggregate deployment-runtime gate to 41 local runtime/deployment components and 28 nested runtime/helper components while preserving no exporter sessions, log shipping, alert delivery, public exposure, service-manager actions, external calls, sensitive-material loading, live execution, signing, broadcasts, or readiness claims.
- Phase 115 update: The deployment evidence bundle now includes `deployment-host-dashboard-runtime` as a direct bounded local component, and the deployment evidence checklist requires it before reporting zero missing required components. This preserves no persistent dashboard server, public exposure, browser credential handling, service-manager actions, external calls, live controls, live execution, or readiness claims.
- Phase 116 update: Deployment-facing runtime/evidence gates now require the existing local dashboard loopback runtime probe through `scripts/validate_deployment_host_runtime.py --run-dashboard-loopback-runtime`, `scripts/validate_deployment_runtime_gate.py`, `scripts/validate_deployment_evidence_bundle.py`, and `scripts/validate_deployment_evidence_checklist.py`, raising the aggregate deployment-runtime gate to 42 local runtime/deployment components and 29 nested runtime/helper components while preserving no persistent dashboard server, public exposure, service-manager action, external calls, live controls, live execution, or readiness claims.
- Phase 117 update: Deployment-facing runtime/evidence gates now require the existing local dashboard session lifecycle validation through `scripts/validate_deployment_host_runtime.py --run-dashboard-session-lifecycle`, `scripts/validate_deployment_runtime_gate.py`, `scripts/validate_deployment_evidence_bundle.py`, and `scripts/validate_deployment_evidence_checklist.py`, raising the aggregate deployment-runtime gate to 43 local runtime/deployment components and 30 nested runtime/helper components while preserving no persistent dashboard server, browser credentials, cookie or CSRF token material, public exposure, service-manager action, external calls, live controls, live execution, or readiness claims.
- Phase 118 update: Deployment-facing runtime/evidence gates now require the existing local communications outbox validation through `scripts/validate_deployment_host_runtime.py --run-communications-outbox`, `scripts/validate_deployment_runtime_gate.py`, `scripts/validate_deployment_evidence_bundle.py`, and `scripts/validate_deployment_evidence_checklist.py`, raising the aggregate deployment-runtime gate to 44 local runtime/deployment components and 31 nested runtime/helper components while preserving no real provider delivery, outbound network use, platform-token loading, service-manager action, external calls, live execution, signing, broadcasts, or readiness claims.
- Phase 119 update: The hardening-core aggregate gate now requires `arb-agent validate-secret-backup-restore --workspace <fresh-dir>` as a fifth component and asserts sanitized references, backup payload shape verification, restore verification, review-window validity, fail-closed audit/state behavior, two replayed audit records, SQLite checkpoint recovery, and no secret material loading, plaintext decryption, keystore writes, external secret restore, signing/broadcast, or readiness claims.
- Phase 120 update: The hardening-core aggregate gate now requires `arb-agent validate-withdrawal-policy-boundary --workspace <fresh-dir>` as a sixth component and asserts config, strategy flag, strategy intent, trust-contract, destination allowlist, signing-boundary, audit/state fail-closed, audit replay, and SQLite checkpoint recovery guards while preserving no external submission, secret recording, withdrawal execution, or readiness claims.
- Phase 121 update: The hardening-core aggregate gate now requires `arb-agent validate-signer-boundary-audit --workspace <fresh-dir>` as a seventh component and asserts rejected unavailable signer requests, local signer-scope review readiness, signer request and scope audit fail-closed behavior, state fail-closed behavior, audit replay, SQLite checkpoint recovery, and no signer material loading, plaintext decryption, signing, broadcast, RPC call, or readiness claims.
- Phase 122 update: The hardening-core aggregate gate now requires `arb-agent validate-destination-boundary-audit --workspace <fresh-dir>` as an eighth component and asserts destination allowlist version and reference accounting, destination allowlist and ownership-review audit fail-closed behavior, state fail-closed behavior, audit replay, SQLite checkpoint recovery, and no ownership proof execution, signer material loading, challenge signing, or readiness claims.
- Phase 123 update: The hardening-core aggregate gate now requires `scripts/validate_execution_path_gate.py --json` as a ninth component and asserts the 18-component local execution-path aggregate passes without external calls, external submission, signer material loading, plaintext decryption, signing, broadcast, live execution, or readiness claims.
- Phase 124 update: The hardening-core aggregate gate now requires `scripts/validate_operator_surface_gate.py --json` as a tenth component and asserts the 15-component local operator-surface aggregate passes without outbound network use, public network exposure, service-manager action, external submission, signing or broadcast, live execution, or readiness claims.
- Phase 125 update: The hardening-core aggregate gate now requires `scripts/validate_opportunity_scenario_gate.py --json` as an eleventh component and asserts the 14-component local opportunity-scenario aggregate passes with replay-latency and validation-coverage review enforcement, candidate coverage, and no external calls, external data downloads, adapter submission, external fuzzer invocation, live network use, signing or broadcast, live execution, or readiness claims.
- Phase 126 update: The hardening-core aggregate gate now requires `scripts/validate_connector_scenario_gate.py --json` as a twelfth component and asserts the 25-component local connector-scenario aggregate passes with audit replay coverage, market-data/fee/Web3 enforcement flags, and no live network use, credential loading, WebSocket opening, live provider calls, external submission, RPC calls, signing or broadcast, live execution, or readiness claims.
- Phase 127 update: The hardening-core aggregate gate now requires `scripts/validate_deployment_evidence_checklist.py --json` as a thirteenth component and asserts the local deployment evidence checklist passes with 34 bundle components, zero missing required components, preserved external-evidence blockers, and no service actions, file mutation, secret loading, external calls, alerts, live execution, embedded artifact contents, or readiness claims.
- Phase 128 update: The dashboard subsystem now exposes a typed persistent local host readiness review with append-only audit and SQLite WAL checkpoint persistence, `arb-agent validate-dashboard-persistent-local-host --workspace <fresh-dir>`, and operator-surface aggregate enforcement as the sixteenth local operator component. The gate validates local host/auth/session/control prerequisites without starting a persistent dashboard server, exposing a public listener, retaining browser/session/CSRF secret material, enabling live controls, performing external submission, or claiming production readiness.
- Phase 129 update: The communications subsystem now exposes a typed provider adapter readiness review with append-only audit and SQLite WAL checkpoint prerequisite recovery through `arb-agent validate-communications-provider-adapter-readiness --workspace <fresh-dir>`, and operator-surface aggregate enforcement as the seventeenth local operator component. The gate validates local non-secret adapter plan shape after provider submission preflight while keeping real provider validation evidence explicitly missing and blocked.
...
- `arb-core::fees` defines fee schedules, fee estimates, fee-adjusted edge calculation, liquidity-role classification, reference-only fee verification records, local fee schedule reconciliation review records, local fee live-provider boundary review records, audit/state checkpoint helpers for fee verification reports, and a read-only `FeeProvider` trait.
