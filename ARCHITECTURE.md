...
- Status: Phase 48 local deterministic Web3 sandbox/live discrepancy calibration implemented after Phase 47 local deterministic Web3 broadcast adapter control review after Phase 46 local deterministic Web3 raw transaction serialization review after Phase 45 local deterministic Web3 provider nonce reconciliation after Phase 44 local deterministic Web3 unsigned transaction construction after Phase 43 local deterministic Web3 broadcast-readiness review, Phase 42 local deterministic Web3 unsigned payload review, Phase 41 local deterministic Web3 nonce reservation, Phase 40 local deterministic Web3 pre-sign safety review, Phase 39 local deterministic signer authorization envelope, Phase 38 local deterministic signer runtime isolation review, Phase 37 local deterministic DEX/Web3 protocol risk review, Phase 36 local deterministic DEX/Web3 transaction lifecycle transcript parsing, Phase 35 local deterministic CEX balance snapshot transcript parsing, Phase 34 local deterministic CEX order lifecycle transcript parsing, Phase 33 local deterministic DEX/Web3 response transcript parsing, Phase 32 local deterministic DEX/Web3 request-plan validation, Phase 31 local deterministic CEX market-data request-plan validation, Phase 30 local deterministic connector scenario aggregate gate, Phase 29 local deterministic opportunity scenario aggregate gate, and Phase 28 local deterministic deployment-runtime aggregate gate. Phase 48 adds typed local Web3 sandbox/live discrepancy calibration for broadcast-control prerequisite coherence, sanitized sandbox/live observation references, minimum sample counts, price/latency/fee discrepancy bounds, audit replay, and SQLite checkpoint recovery without external calls, credential loading, RPC calls, signer material loading, signing, broadcasting, live execution, or claiming custody readiness. Phase 47 adds typed local Web3 broadcast adapter control review for raw-transaction-serialization prerequisite coherence, adapter/operator/audit-state references, kill-switch, rate-limit, replay/idempotency controls, broadcast-permission denial, audit replay, and SQLite checkpoint recovery without RPC calls, signer material loading, signing, broadcasting, live execution, or claiming custody readiness. Phase 46 adds typed local Web3 raw transaction serialization review for provider-nonce prerequisite coherence, transaction type, chain id, fee field, and access-list references, raw-byte denial, raw-calldata denial, raw-serialization denial, audit replay, and SQLite checkpoint recovery without RPC calls, signer material loading, signing, broadcasting, live execution, or claiming custody readiness. Phase 45 adds typed local Web3 provider nonce reconciliation for caller-supplied provider snapshot references, provider next nonce, pending nonce uniqueness, construction nonce coherence, snapshot freshness, audit replay, and SQLite checkpoint recovery without RPC calls, signer material loading, signing, broadcasting, live execution, or claiming custody readiness. Phase 44 adds typed local Web3 unsigned transaction construction for broadcast-readiness prerequisite coherence, sanitized payload/selector/digest/nonce/gas metadata, raw-calldata denial, raw-transaction serialization denial, audit replay, and SQLite checkpoint recovery without RPC calls, signer material loading, signing, broadcasting, live execution, or claiming custody readiness. Phase 43 adds typed local Web3 broadcast-readiness review for unsigned-payload/pre-sign prerequisite coherence, non-secret signer authorization/live adapter/operator approval references, broadcast-permission denial, audit replay, and SQLite checkpoint recovery without RPC calls, signer material loading, signing, broadcasting, live execution, or claiming custody readiness. Phase 42 adds typed local Web3 unsigned payload review for payload hash/label matching, nonce reservation readiness, router/spender coherence, gas cap metadata, raw-calldata denial, audit replay, and SQLite checkpoint recovery without raw calldata generation, calling RPC, loading signer material, signing, broadcasting, live execution, or claiming custody readiness. Phase 41 adds typed local Web3 nonce reservation for caller-supplied nonce metadata, stale nonce denial, duplicate in-flight nonce denial, already-reserved nonce denial, audit replay, and SQLite checkpoint recovery without calling RPC, loading signer material, signing, broadcasting, live execution, or claiming custody readiness. Current workspace Rust/CI validation evidence exists, while live exchange/RPC adapter implementation, real provider-backed nonce retrieval, provider-backed validation, broader external/deployment scenario-corpus validation, deployment-host audit validation under service lifecycle, physical disk-full evidence, deployment-host retention/rotation execution evidence, operator-controlled service-manager lifecycle execution evidence, rollback execution evidence, incident-response execution evidence, external sandbox/live calibration evidence, deployment-host durability validation, external agent execution, and real production hardening validation remain deferred.
...
- `migrate_config_toml_to_current` validates current configs and upgrades known local legacy `[markets]` and `[notifications]` aliases into the current non-secret schema without loading secret material or enabling live execution.
- Live-armed strategy scope, withdrawals, bridges, and flashloans are denied in this boundary.
- Profitability tuning, broader replay validation, and external calibration remain future work.
...
- `append_paper_execution_report_audit`, `append_paper_ledger_entry_audit`, and audited ledgered execution helpers append paper report plus reserve/settlement mutation records to the local append-only audit journal and reopen it for hash-chain replay checks.
- `ledger_execution_adapter_run_paper_fills` replays adapter reconciliation records against modeled fills, settles local deterministic execution-adapter modeled fills into the paper ledger, appends the paper report plus reserve/settlement audit records, persists the final ledger checkpoint, rejects duplicate modeled-fill settlement after SQLite checkpoint reopen before ledger/audit mutation, and refuses live/external-submission records.
...
- Does not replace future mandatory audit-before-action journal integration, external sandbox/live realism checks, or external production-host durability validation
...
- Local DEX/Web3 protocol risk review records check caller-supplied spender hygiene, allowance denial, approval revocation planning, gas/slippage caps, MEV controls, token metadata review, and protocol terms review without RPC calls, signer material, signing, broadcasts, bridges, live execution, or production-readiness claims.
...
- Local protocol risk review checks caller-supplied metadata for token metadata, slippage, MEV risk, gas, and protocol terms, but does not validate router contracts, token metadata, slippage, MEV risk, gas, or protocol terms externally
...
- CEX framework has local named exchange fixture matching, local Binance/Coinbase/Kraken REST/WebSocket market-data request plans, mocked order-book transcript parsing, local balance snapshot transcript parsing, local rate-limit validation, and local credential/API-scope review only and is not connected to live REST/WebSocket APIs, account queries, or sandboxes
...
- Opportunity engine has current workspace Rust validation evidence and now models local caller-supplied order-book depth, paper inventory caps, transfer-risk penalties, same-venue triangular path discovery, local replay/false-positive reports, a built-in local regression corpus, local historical fixture replay aggregation, local replay CLI validation commands, local candidate audit/state traces, local candidate trace restart/reopen recovery validation, and replay-candidate planner handoff validation wired into CI, but broader external/deployment scenario-corpus validation and external sandbox/live calibration remain missing
