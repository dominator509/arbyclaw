...
- Phase 95 update: The CEX framework now exposes `CexLiveAdapterBoundaryReviewRequest` / `CexLiveAdapterBoundaryReviewReport` plus `arb-agent validate-cex-live-adapter-boundary`, and the connector scenario aggregate gate now includes 20 local components with the CEX live-adapter boundary enforced while preserving no REST calls, WebSocket connections, credential loading, external submission, live execution, or production-readiness claim.
